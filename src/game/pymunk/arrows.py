...
from pymunk import Poly, Body, PivotJoint, GearJoint
from pymunk.vec2d import Vec2d


# returns the physical body of an arrow.
def create_arrow():
    vs = [(-30, 0), (0, 3), (10, 0), (0, -3)]
    # mass = 1
    # moment = pymunk.moment_for_poly(mass, vs)
    arrow_body: Body = pymunk.Body(body_type=pymunk.Body.KINEMATIC)

    arrow_shape: Poly = pymunk.Poly(arrow_body, vs)
...
def stick_arrow_to_target(space, arrow_body, target_body, position, flying_arrows):
    pivot_joint: PivotJoint = pymunk.PivotJoint(arrow_body, target_body, position)
    phase = target_body.angle - arrow_body.angle
    gear_joint: GearJoint = pymunk.GearJoint(arrow_body, target_body, phase, 1)
